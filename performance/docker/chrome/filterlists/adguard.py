...
# logging.basicConfig(filename='/data/measurement.log', level=logging.INFO)
...
def verify_selected(webdriver, extension_id: str, names: list[str]|bool):
    ...
    names_lower_case = [name.lower() for name in names] if isinstance(names, list) else []
    unmentioned = []
    ...
    for group in range(1, 8):
        webdriver.get(f"chrome-extension://{extension_id}/pages/options.html#filters?group={group}")
    ...
    if isinstance(names, list) and any(inconsistencies.values()):
        ...
        print("NOT FOUND", [name for name in names_lower_case if inconsistencies[name] == "NOT FOUND"])
        print("SHOULD BE CHECKED", [name for name in names_lower_case if inconsistencies[name] == True])
        print("SHOULD BE UNCHECKED", [name for name in inconsistencies.keys() if inconsistencies[name] == True and not any(keys_match(key, name) for key in names_lower_case)])
    ...
    if isinstance(names, bool) and names and not all(inconsistencies.values()):
        ...
        raise ValueError("Inconsistencies found")
            ...
            # print(title.lower(), names_lower_case, any(name.startswith(title.lower()) for name in names_lower_case))
            
            
            # if select_all or (isinstance(names, list) and any(title.lower().startswith(name) for name in names_lower_case)):
            #     if not checked:
...
def accept_modal(webdriver):
    ...
    if modal:
        ...
        modal = modal[0]
        accept_button = webdriver.find_elements(By.CLASS_NAME, ".ReactModalPortal button--green")
...
def select_by_names(webdriver, extension_id: str, names: list[str]|bool):
    ...
    for group in range(1, 8):
        ...
        title_container_setting_box = webdriver.find_element(By.CSS_SELECTOR, ".title__container .setting__container")
        
        checked = webdriver.execute_script('return arguments[0].querySelector("input").checked;', title_container_setting_box)
        
        print(f"GROUP {group} IS", "ACTIVE" if checked else "INACTIVE")
        
        if not checked:
            print("ACTIVATING GROUP")
            title_container_setting_box.find_element(By.CLASS_NAME,"checkbox__label").click()
        ...
        if select_all:
            ...
            if activated:
                ...
                print("FAILED TO SELECT ALL, FALLING BACK TO ACTIVATING BY NAME IN BATCH")
        ...
        for title in current_activations:
            ...
            if select_all or (isinstance(names, list) and any(keys_match(title, name) for name in names_lower_case)):
        ...
        if titles_to_activate:
            ...
            if not activated:
                print("FAILED TO ACTIVATE IN BATCH, FALLING BACK TO INDIVIDUAL SELECTION")
            else:
                continue
        ...
        for elem in webdriver.find_elements(By.CLASS_NAME, "setting-checkbox"):
            ...
            checked = webdriver.execute_script('return arguments[0].querySelector("input").checked;', elem)
